...
class ControllerModulepavblogcategory extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('pavblogcategory', $this->request->post);		
		...
		$this->data['entry_tabs'] = $this->language->get('entry_tabs');
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pavblogcategory', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/pavblogcategory', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['pavblogcategory_module'])) {
			$this->data['modules'] = $this->request->post['pavblogcategory_module'];
		} elseif ($this->config->get('pavblogcategory_module')) { 
			$this->data['modules'] = $this->config->get('pavblogcategory_module');
		}	
		...
		foreach( $this->data['modules'] as $k=>$m  ){
			$this->data['modules'][$k]['menus'] = $this->model_pavblog_menu->getDropdown(null, $m['category_id'], 'pavblogcategory_module['.($k).'][category_id]' );
		}
		...
		$this->data['tabs'] = $tabs;
		$this->template = 'module/pavblogcategory.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/pavblogcategory')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['pavblogcategory_module'])) {
			foreach ($this->request->post['pavblogcategory_module'] as $key => $value) {
