...
class ControllerModulePavbloglatest extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('pavbloglatest', $this->request->post);		
		...
		$this->data['entry_tabs'] = $this->language->get('entry_tabs');
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pavbloglatest', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/pavbloglatest', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['pavbloglatest_module'])) {
			$this->data['modules'] = $this->request->post['pavbloglatest_module'];
		} elseif ($this->config->get('pavbloglatest_module')) { 
			$this->data['modules'] = $this->config->get('pavbloglatest_module');
		}	
		...
		$this->data['tabs'] = $tabs;
		$this->template = 'module/pavbloglatest.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/pavbloglatest')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['pavbloglatest_module'])) {
			foreach ($this->request->post['pavbloglatest_module'] as $key => $value) {
