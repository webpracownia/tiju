	...
	public function ajax()
	{
		...
		if( isset($this->request->get['keyword']) ) {
			...
			if( strlen($keywords) >= 2 ) {
				...
				$add = substr( $add, 4 );
				$sql  = 'SELECT pd.product_id, pd.name, p.model, p.image, p.price FROM ' . DB_PREFIX . 'product_description AS pd ';
				...
				if( $res ) {
					...
					foreach( $data as $key => $values ) {
						...
						$product_info['price'] = $values['price'];
						
						$this->load->model('tool/image');
						
						$data[$key] = array(
							'name' => htmlspecialchars_decode($values['name'], ENT_QUOTES),
							'image' => '<img src="'.$this->model_tool_image->resize($values['image'], 50, 50).'" align="absmiddle"/> ',
